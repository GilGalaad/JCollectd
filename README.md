...
```
nohup /jdk1.8/bin/java -Xmx16m -jar JCollectd.jar jcollectd.properties > jcollectd.log &
```
...
Worths to say that:
* the memory footprint is as low as can be, 6 MB (which is the minimum possible) of heap size is enough to let the daemon run with a reasonable configuration, but limiting heap size to something more would alleviate pressure on garbage collection.
