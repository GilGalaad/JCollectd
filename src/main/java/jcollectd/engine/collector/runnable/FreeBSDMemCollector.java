...
@Log4j2
public class FreeBSDMemCollector extends CollectorRunnable {
    ...
    public MemRawSample call() throws Exception {
         ...
         */
        try {
            List<String> stdout = processRunner(List.of("sysctl", "vm.stats.vm.v_page_size", "vm.stats.vm.v_active_count", "vm.stats.vm.v_inactive_count", "vm.stats.vm.v_laundry_count", "vm.stats.vm.v_wire_count", "vm.stats.vm.v_cache_count", "vfs.bufspace", "kstat.zfs.misc.arcstats.size"));
         ...
         */
        try {
            List<String> stdout = processRunner(List.of("swapinfo", "-k"));
            for (var line : stdout) {
                line = line.trim();
            ...
            Thread.currentThread().interrupt();
            throw new RuntimeException("Interrupted while running swapinfo", ex);
        } catch (Exception ex) {
            throw new RuntimeException("Unexpected error while running swapinfo", ex);
        }
